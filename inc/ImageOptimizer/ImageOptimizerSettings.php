...
class ImageOptimizerSettings {
    public function __construct() {
        add_action('admin_menu', [$this, 'add_admin_page']);
        add_action('admin_init', [$this, 'register_settings']);
        add_action('admin_enqueue_scripts', [$this, 'enqueue_admin_assets']);
        add_action('wp_ajax_ImageOptimizer_batch_convert', [$this, 'ajax_batch_convert']);
        add_action('wp_ajax_ImageOptimizer_get_log', [$this, 'ajax_get_log']);
    }

    public function add_admin_page() {
        add_submenu_page(
            \SFX\Options\AdminOptionPages::$menu_slug,
            __('ImageOptimizer Tools', 'sfxtheme'),
            __('ImageOptimizer Tools', 'sfxtheme'),
            'manage_options',
            'ImageOptimizer-tools',
            [$this, 'render_admin_page']
        );
    }

    public function register_settings() {
        register_setting('ImageOptimizer_options', 'webp_max_widths');
        register_setting('ImageOptimizer_options', 'webp_max_heights');
        register_setting('ImageOptimizer_options', 'webp_resize_mode');
        register_setting('ImageOptimizer_options', 'webp_quality');
        register_setting('ImageOptimizer_options', 'webp_batch_size');
        register_setting('ImageOptimizer_options', 'webp_preserve_originals');
        register_setting('ImageOptimizer_options', 'webp_disable_auto_conversion');
        register_setting('ImageOptimizer_options', 'webp_min_size_kb');
        register_setting('ImageOptimizer_options', 'webp_use_avif');
    }

    public function enqueue_admin_assets($hook) {
        if (strpos($hook, 'ImageOptimizer-tools') === false) return;
        wp_enqueue_script('ImageOptimizer-admin', get_stylesheet_directory_uri() . '/assets/js/ImageOptimizer-admin.js', ['jquery'], null, true);
        wp_localize_script('ImageOptimizer-admin', 'ImageOptimizerAjax', [
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce'    => wp_create_nonce('ImageOptimizer_nonce'),
        ]);
    }
    ...
    public function render_admin_page() {
        ?>
        <div class="wrap">
            <h1><?php _e('ImageOptimizer Tools', 'sfxtheme'); ?></h1>
            <form method="post" action="options.php">
                <?php
                settings_fields('ImageOptimizer_options');
                do_settings_sections('ImageOptimizer_options');
                ?>
            ...
            <hr />
            <button id="ImageOptimizer-batch-convert" class="button button-primary"><?php _e('Batch Convert Images', 'sfxtheme'); ?></button>
            <pre id="ImageOptimizer-log" style="margin-top:2em; background:#f7f7f7; padding:1em; max-height:300px; overflow:auto;"></pre>
        </div>
        <?php
    }

    public function ajax_batch_convert() {
        check_ajax_referer('ImageOptimizer_nonce', 'nonce');
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => __('Permission denied', 'sfxtheme')]);
        }
        // Dummy batch conversion logic (replace with real logic)
        $log = get_option('ImageOptimizer_conversion_log', []);
        $log[] = date('Y-m-d H:i:s') . ': Batch conversion started.';
        update_option('ImageOptimizer_conversion_log', array_slice((array)$log, -500));
